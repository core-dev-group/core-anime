...
export default function GenresPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="container mx-auto px-3 sm:px-6 py-4 sm:py-8 min-h-screen">
      {/* Header */}
      <div className="mb-6 sm:mb-10 flex flex-col md:flex-row md:items-end justify-between gap-3 sm:gap-4 border-b-2 border-surface-soft pb-3 sm:pb-4 animate-fade-in-up">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Link to="/home" className="inline-block bg-surface text-foreground/70 hover:text-accent hover:border-accent transition-colors px-2.5 sm:px-3 py-1 font-bold tracking-widest text-xs border-2 border-surface-soft">
      ...
      {/* Grid */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2.5 sm:gap-4 animate-fade-in-up animation-delay-100">
        {genreList.map((genre: any, index: number) => (
          <Link
            key={genre.genreId}
            to={`/genre/${genre.genreId}`}
            className="group block bg-surface border-2 border-surface-soft hover:border-accent p-2.5 sm:p-4 text-center transition-all duration-300 shadow-[2px_2px_0px_rgba(46,78,78,0.5)] sm:shadow-[4px_4px_0px_rgba(46,78,78,0.5)] hover:shadow-[4px_4px_0px_rgba(255,59,59,0.3)] hover:-translate-y-0.5"
            style={{ animationDelay: `${(index % 20) * 30}ms` }}
          >
            <span className="font-display text-foreground group-hover:text-accent text-sm sm:text-lg uppercase tracking-wider transition-colors block truncate">
              {genre.title}
            </span>
          </Link>
        ))}
