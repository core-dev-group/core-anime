...
export const oploverz: AnimeProvider = {
  name: "oploverz",
  isHealthy: async () => await isProviderHealthy("oploverz"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/oploverz/home');
    ...
    return list.map((item: any) => toUnifiedAnime("oploverz", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/oploverz/search/' + encodeURIComponent(query));
