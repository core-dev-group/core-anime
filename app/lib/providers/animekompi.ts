...
export const animekompi: AnimeProvider = {
  name: "animekompi",
  isHealthy: async () => await isProviderHealthy("animekompi"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/animekompi/home');
    ...
    return list.map((item: any) => toUnifiedAnime("animekompi", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/animekompi/search?q=' + encodeURIComponent(query));
