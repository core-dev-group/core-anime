...
export const kusonime: AnimeProvider = {
  name: "kusonime",
  isHealthy: async () => await isProviderHealthy("kusonime"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/kusonime/latest');
    ...
    return list.map((item: any) => toUnifiedAnime("kusonime", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/kusonime/search/' + encodeURIComponent(query));
