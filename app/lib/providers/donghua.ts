...
export const donghua: AnimeProvider = {
  name: "donghua",
  isHealthy: async () => await isProviderHealthy("donghua"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/donghua/home/1');
    ...
    return list.map((item: any) => toUnifiedAnime("donghua", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/donghua/search/' + encodeURIComponent(query));
