...
export const stream: AnimeProvider = {
  name: "stream",
  isHealthy: async () => await isProviderHealthy("stream"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/stream/latest');
    ...
    return list.map((item: any) => toUnifiedAnime("stream", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/stream/search/' + encodeURIComponent(query));
