...
export const donghub: AnimeProvider = {
  name: "donghub",
  isHealthy: async () => await isProviderHealthy("donghub"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/donghub/home');
    ...
    return list.map((item: any) => toUnifiedAnime("donghub", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/donghub/search/' + encodeURIComponent(query));
