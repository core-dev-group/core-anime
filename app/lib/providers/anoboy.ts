...
export const anoboy: AnimeProvider = {
  name: "anoboy",
  isHealthy: async () => await isProviderHealthy("anoboy"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/anoboy/home');
    ...
    return list.map((item: any) => toUnifiedAnime("anoboy", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/anoboy/search/' + encodeURIComponent(query));
