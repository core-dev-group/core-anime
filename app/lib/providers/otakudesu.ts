...
export const otakudesu: AnimeProvider = {
  name: "otakudesu",
  isHealthy: async () => await isProviderHealthy("otakudesu"),
  ...
  getHome: async (page = 1) => {
    ...
    return list.map((item: any) => toUnifiedAnime("otakudesu", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/search/' + encodeURIComponent(query));
  ...
  getSchedule: async () => {
    ...
    return scheduleData.map((dayObj: any) => ({
      ...
      animeList: (dayObj.anime_list || dayObj.animeList || []).map((item: any) => toUnifiedAnime("otakudesu", {
        title: item.title || item.name || "Unknown",
        thumbnail: item.thumb || item.thumbnail || item.poster || item.image || "",
        sourceSlug: item.slug || item.endpoint || item.id || "",
        status: item.status,
