...
export const winbu: AnimeProvider = {
  name: "winbu",
  isHealthy: async () => await isProviderHealthy("winbu"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/winbu/home');
    ...
    return list.map((item: any) => toUnifiedAnime("winbu", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/winbu/search?q=' + encodeURIComponent(query));
