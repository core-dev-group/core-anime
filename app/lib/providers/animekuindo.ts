...
export const animekuindo: AnimeProvider = {
  name: "animekuindo",
  isHealthy: async () => await isProviderHealthy("animekuindo"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/animekuindo/home');
    ...
    return list.map((item: any) => toUnifiedAnime("animekuindo", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/animekuindo/search/' + encodeURIComponent(query));
