...
export const nimegami: AnimeProvider = {
  name: "nimegami",
  isHealthy: async () => await isProviderHealthy("nimegami"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/nimegami/home');
    ...
    return list.map((item: any) => toUnifiedAnime("nimegami", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/nimegami/search/' + encodeURIComponent(query));
