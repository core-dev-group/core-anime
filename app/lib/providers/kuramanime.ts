...
export const kuramanime: AnimeProvider = {
  name: "kuramanime",
  isHealthy: async () => await isProviderHealthy("kuramanime"),
  
  getHome: async (page = 1) => {
    const data = await fetchApi('/anime/kura/home');
    ...
    return list.map((item: any) => toUnifiedAnime("kuramanime", {
  ...
  search: async (query: string, page = 1) => {
    const data = await fetchApi('/anime/kura/search/' + encodeURIComponent(query));
